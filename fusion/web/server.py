...
SAMPLES_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'samples')
...
def get_samples():
    ...
    try:
        files = [f for f in os.listdir(SAMPLES_FOLDER) 
...
def fuse_images():
    try:
        ...
        if use_samples:
            ...
            a_img = Image.open(os.path.join(SAMPLES_FOLDER, filename_a)).convert("RGB")
            b_img = Image.open(os.path.join(SAMPLES_FOLDER, filename_b)).convert("RGB")
        else:
